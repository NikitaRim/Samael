...
class HomeworkResult():
    def __init__(self, author: Student, task: Homework, solution: str):
        ...
        if not isinstance(task, Homework):
            raise TypeError("Task doesn't belong to the Homework class")

class Teacher(Person):
    homework_done = {}
    def create_homework(self, text: str, deadline: int):
        return Homework(text, datetime.timedelta(float(deadline)))
    def check_homework(self, homework_result: HomeworkResult):
        if len(homework_result.solution) > 5:
            ...
            return False
    def reset_resulrt(self, homework: Homework = None):
